...
/* in fact this structure is derived (in IEC) from EventStruct so all components are located at the beginning */
typedef struct _tagEventStruct2
{
	EventStruct baseEvent;
	RTS_IEC_LWORD  lwIdStack;
} EventStruct2;

typedef struct _tagEventExtensionData
{
	RTS_SIZE size;
	void* pData;
} EventExtensionData;
