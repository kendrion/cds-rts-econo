...
#undef CAA_LIBS_IMP
		...
		*/
		...
		#define CL2_QUEUE_ADAPTION 1 /*Enable RxQueue extensions if necessary*/
	#endif
	...
	#ifndef CL2_USE_SEMAPHORES
		#ifdef SYSSEM_NOTIMPLEMENTED
			/* CL2 Lock uses CAA Lock */
			#define CL2_USE_SEMAPHORES 0
		#else
			/* CL2 Lock uses semaphores */
			#define CL2_USE_SEMAPHORES 1
		#endif
	#endif

	#if CL2_USE_SEMAPHORES
		/* If semaphores are implemented use semaphores for locking mechanism (recommended!). */
		#define CL2_LOCK RTS_HANDLE _rts_lock;
		#define CL2_CREATE_LOCK() CAL_SysSemCreate((RTS_RESULT*)0)
		#define CL2_ENTER_LOCK(h) (void)CAL_SysSemEnter(h)
		#define CL2_LEAVE_LOCK(h) (void)CAL_SysSemLeave(h)
		#define CL2_DELETE_LOCK(h) (void)CAL_SysSemDelete(h)
	#else
		/* If no semaphores are available ==> Use locking mechanism defined by platform (e.g. IRQ lock )*/
		#define CL2_LOCK CAA_LOCK
		#define CL2_CREATE_LOCK() CAA_CREATE_LOCK()
		#define CL2_ENTER_LOCK(h) CAA_ENTER_LOCK(h)
		#define CL2_LEAVE_LOCK(h) CAA_LEAVE_LOCK(h)
		#define CL2_DELETE_LOCK(h) CAA_DELETE_LOCK(h)
	#endif
#endif
