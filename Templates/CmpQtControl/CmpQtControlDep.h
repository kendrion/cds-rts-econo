 ...
 *  </copyright>
 */
#ifndef _CMPQTCONTROLDEP_H_
#define _CMPQTCONTROLDEP_H_

#define COMPONENT_NAME "CmpQtControl" COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpQtControl)
#define COMPONENT_NAME_UNQUOTED CmpQtControl
...
#include "SysNativeControlItf.h"
    ...
    {\
        ...
        if (pISysNativeControl == NULL && s_pfCMCreateInstance != NULL) \
        { \
            pIBase = (IBase *)s_pfCMCreateInstance(CLASSID_CSysNativeControl, &initResult); \
            if (pIBase != NULL) \
            { \
                pISysNativeControl = (ISysNativeControl *)pIBase->QueryInterface(pIBase, ITFID_ISysNativeControl, &initResult); \
                pIBase->Release(pIBase); \
            } \
        } \
    ...
    {\
        ...
        if (pISysNativeControl != NULL) \
        { \
            pIBase = (IBase *)pISysNativeControl->QueryInterface(pISysNativeControl, ITFID_IBase, &exitResult); \
            if (pIBase != NULL) \
            { \
                 pIBase->Release(pIBase); \
                 if (pIBase->Release(pIBase) == 0) /* The object will be deleted here! */ \
...
#ifndef CMPQTCONTROL_DISABLE_EXTREF
#define EXPORT_EXTREF_STMT \
          
#else
#define EXPORT_EXTREF_STMT
#endif
#ifndef CMPQTCONTROL_DISABLE_EXTREF2
#define EXPORT_EXTREF2_STMT \
          
#else
#define EXPORT_EXTREF2_STMT
#endif
#if !defined(CMPQTCONTROL_DISABLE_CMPITF) && !defined(STATIC_LINK) && !defined(CPLUSPLUS) && !defined(CPLUSPLUS_ONLY)
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpQtControl__Entry
#endif


#ifdef CPLUSPLUS

class CCmpQtControl : public ICmpEventCallback 
{
    public:
        CCmpQtControl() : hCmpQtControl(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpQtControl()
        {
        }
    ...
    public:
        RTS_HANDLE hCmpQtControl;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpQtControl Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpQtControl_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpQtControl_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
