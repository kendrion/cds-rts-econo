 ...
 *	USAGE:
 * 
 *	Install the device description to the CODESYS device repository and use "Add Device" command to add the device "IoDrvVerySimple" to your top level device (PLC).
 ...
 *	Module: 		Very simple IO driver example
 *	Version:
******************************************************************************/
...
static IoDrvVerySimple s_IoDrvVerySimple;
...
*/
{
	...
	if (pInitStruct->pfCMRegisterClass != NULL)
		pInitStruct->pfCMRegisterClass(COMPONENT_ID, CLASSID_CIoDrvVerySimple);
...
{
	if (cid == CLASSID_CIoDrvVerySimple)
	{
		IoDrvVerySimple *pIoDrvVerySimple = &s_IoDrvVerySimple;
		IBase *pI = &pIoDrvVerySimple->Base;
		pIoDrvVerySimple->pIBase = &pIoDrvVerySimple->Base;
		pI->bIEC = 0;
		pI->AddRef = AddRef;
		pI->Release = Release;
...
static unsigned char s_uInputs[MAX_BYTES];
static unsigned char s_uOutputs[MAX_BYTES];

STATICITF RTS_HANDLE CDECL IoDrvCreate(RTS_HANDLE hIIoDrv, CLASSID ClassId, int iId, RTS_RESULT *pResult)
{
	IoDrvInfo *pInfo;
	IoDrvVerySimple *pIoDrvVerySimple;
	...
	pInfo->hSpecific = (RTS_IEC_HANDLE)0;
	pInfo->dwVersion = CMP_VERSION;

	strcpy(pInfo->szDriverName, "IoDrvVerySimple");
	strcpy(pInfo->szDeviceName, "VerySimpleDevice");
	...
	return (RTS_HANDLE)pIoDrvVerySimple;
}
...
STATICITF RTS_RESULT CDECL IoDrvUpdateConfiguration(RTS_HANDLE hIoDrv, IoConfigConnector *pConnectorList, int nCount)
{
	...
	*/
	IoConfigConnector *pConnector = CAL_IoMgrConfigGetFirstConnector(pConnectorList, &nCount, 32769);
	IoDrvVerySimple *pIoDrvVerySimple = (IoDrvVerySimple *)(hIoDrv); /* hIoDrv as returned from IoDrvCreate */
	IBase *pIBase = pIoDrvVerySimple->pIBase;
	/*ICmpIoDrv *pIIoDrv = pIoDrvVerySimple->pIoDrv;*/
	...
	if (pConnector != NULL)
	{
		...
		if (pConnector->hIoDrv == 0)
		{
			...
			pParameter = CAL_IoMgrConfigGetParameter(pConnector, 2000000000);
			if (pParameter != NULL)
				pParameter->dwDriverSpecific = (RTS_IEC_BYTE *)s_uInputs;


			/* Output channels */
			pParameter = CAL_IoMgrConfigGetParameter(pConnector, 2000001000);
			if (pParameter != NULL)
				pParameter->dwDriverSpecific = (RTS_IEC_BYTE *)s_uOutputs;
...
/* check parameter IDs -> we can only merge channels from the same parameterId*/
/* this example can only merge same type and length*/
int CheckIfChannelsCanBeMerged(IoConfigChannelMap* pFirstChannel, IoConfigChannelMap* pNextChannel)
{
	if(pFirstChannel == 0 || pNextChannel == 0)
		return 0;

	if((pFirstChannel->pParameter->dwParameterId == pNextChannel->pParameter->dwParameterId) &&
		(pFirstChannel->wParameterBitOffset + pFirstChannel->wSize == pNextChannel->wParameterBitOffset) &&
		(pFirstChannel->wBaseTypeInformation == pNextChannel->wBaseTypeInformation))
	{
			return 1;
	}
...
static IoConfigChannelMap* s_pPrevChannel = 0;	/* always store the last channel here */
static IoConfigChannelMap* s_pFirstChannel = 0; /* store the first channel to merge with here */
STATICITF RTS_RESULT CDECL IoDrvUpdateMapping(RTS_HANDLE hIoDrv, IoConfigTaskMap *pTaskMapList, int nCount)
{
	int i,j,k;
	RTS_UI32 currentParameterId;

	/* search task map */
	for(i=0; i<nCount; i++)
	{
		/* search connector map */
		for(j=0; j<pTaskMapList[i].wNumOfConnectorMap; j++)
		{
			/* search channels */
			for(k=0; k<pTaskMapList[i].pConnectorMapList[j].dwNumOfChannels; k++)
			{
				IoConfigChannelMap* pThisChannel = &pTaskMapList[i].pConnectorMapList[j].pChannelMapList[k];

				/* check if they can be merged */
				if(CheckIfChannelsCanBeMerged(s_pFirstChannel, pThisChannel))
				{
					/* merge them */
					int NrofChannelsLeft = pTaskMapList[i].pConnectorMapList[j].dwNumOfChannels - 1 - k;
					int BytesToCopy = NrofChannelsLeft * sizeof(IoConfigChannelMap);

					/* merge the size */
					s_pFirstChannel->wSize += pThisChannel->wSize;

					/* move the later items one forward */
					if(BytesToCopy)
					{
						memmove(&(pTaskMapList[i].pConnectorMapList[j].pChannelMapList[k]), &(pTaskMapList[i].pConnectorMapList[j].pChannelMapList[k + 1]), BytesToCopy);
						k--;
					}

					/* remove one channel from list */
					pTaskMapList[i].pConnectorMapList[j].dwNumOfChannels--;

				}
				else
				{
					printf("setting first channel: 0x%x\n", pThisChannel);
					s_pFirstChannel = pThisChannel;
				}
				s_pPrevChannel = pThisChannel;


			}

			/* Rewrite the number of channels */
			//pTaskMapList[i].pConnectorMapList[j].dwNumOfChannels = channel;
		}
	}
	return ERR_OK;
	...
	for (i=0; i<nCount; i++)
	{
		for (j=0; j<pConnectorMapList[i].dwNumOfChannels; j++)
		{
			...
			*/
			...
			printf("readinputs: id: %d, channel size: %d\n",pConnectorMapList[i].pChannelMapList[j].pParameter->dwParameterId,  pConnectorMapList[i].pChannelMapList[j].wSize);
		}
	}
	return ERR_OK;
}
...
STATICITF RTS_RESULT CDECL IoDrvWriteOutputs(RTS_HANDLE hIoDrv, IoConfigConnectorMap *pConnectorMapList, int nCount)
{
	...
	for (i=0; i<nCount; i++)
	{
		for (j=0; j<pConnectorMapList[i].dwNumOfChannels; j++)
		{
			...
			printf("writeoutputs: id: %d, channel size: %d\n",pConnectorMapList[i].pChannelMapList[j].pParameter->dwParameterId,  pConnectorMapList[i].pChannelMapList[j].wSize);
		}
	}
	return ERR_OK;
}
...
STATICITF RTS_RESULT CDECL IoDrvStartBusCycle(RTS_HANDLE hIoDrv, IoConfigConnector *pConnector)
{
	...
	memcpy(s_uInputs, s_uOutputs, sizeof(s_uInputs));
