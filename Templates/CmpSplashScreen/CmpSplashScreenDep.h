...
#ifndef _CMPSPLASHSCREENDEP_H_
#define _CMPSPLASHSCREENDEP_H_

#define COMPONENT_NAME "CmpSplashScreen" COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpSplashScreen)
#define COMPONENT_NAME_UNQUOTED CmpSplashScreen
...
#define CMP_VERSION         UINT32_C(0x03050F00)
#define CMP_VERSION_STRING "3.5.15.0"
#define CMP_VERSION_RC      3,5,15,0
...
/**
 * \file CmpSplashScreenItf.h
 */
#include "CmpSplashScreenItf.h"
    ...
    {\
        ...
        if (ERR_OK == importResult ) importResult = GET_SysTargetGetVersion(0);\
          if (ERR_OK == importResult ) importResult = GET_SysTargetGetNodeName(0);\
          ...
          if (ERR_OK == importResult ) importResult = GET_CMUtlVersionToString(0);\
           \
        /* To make LINT happy */\
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpSplashScreen__Entry
#endif


#ifdef CPLUSPLUS

class CCmpSplashScreen : public ICmpSplashScreen 
{
    public:
        CCmpSplashScreen() : hCmpSplashScreen(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpSplashScreen()
        {
        }
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((ICmpSplashScreen *)this);            
            else if (iid == ITFID_ICmpSplashScreen)
                pItf = dynamic_cast<ICmpSplashScreen *>(this); 
            else
            {
                if (pResult != NULL)
    ...
    public:
        RTS_HANDLE hCmpSplashScreen;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpSplashScreen Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpSplashScreen_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpSplashScreen_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
