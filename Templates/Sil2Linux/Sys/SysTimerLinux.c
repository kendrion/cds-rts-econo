...
#define SIL2_LINUX_WATCHDOG_INTERVAL_NS 1000000 // 1ms
#endif
...
extern void SIL2OEMSwitchToSafeCtx_nolock(void);
extern void SIL2OEMSwitchToPreviousCtx_nolock(void);
...
void SysTimerRestoreContextException(void)
{
	...
	for(i=0; i<MAX_TIMERS; i++)
	{
		if(timer[i].opened == 1)
		{
			timer[i].bRunWorker = 0;
			timer[i].bActive = 0;
			timer[i].bRun = 0;

			/* kill all other threads but this */
			if(pthread_self() != timer[i].thread)
			{
				pthread_cancel(timer[i].thread);
				pthread_cancel(timer[i].worker);
			}
			else
				iThisThread = i;
		}
	...
	/* kill this thread */
	pthread_cancel(timer[iThisThread].worker);
	pthread_exit(0);
}
...
static void timerfunction(int i)
{
	...
	localPrio = timer[i].pti->ulPriority * 2;

	if(localPrio < SIL2_LINUX_MAX_PRIO_TIMER)
		sched_param.sched_priority = SIL2_LINUX_MAX_PRIO_TIMER - localPrio;
	else
		sched_param.sched_priority = SIL2_LINUX_MIN_PRIO_TIMER + 1;
	...
	sprintf(pszlocalThreadName, "timer %d", i);
	prctl(PR_SET_NAME,(unsigned long)pszlocalThreadName,0,0);
	...
	while(timer[i].bRun)
	{
		...
		pthread_mutex_lock(&timer[i].workerMutex);
		pthread_cond_signal(&timer[i].workerCond);
		pthread_mutex_unlock(&timer[i].workerMutex);
...
static void workerfunction(int i)
{
	struct sched_param sched_param;
	int localPrio;
	char pszlocalThreadName[12];
	...
	localPrio = timer[i].pti->ulPriority * 2 + 1;
	...
	sprintf(pszlocalThreadName, "worker %d", i);
	prctl(PR_SET_NAME,(unsigned long)pszlocalThreadName ,0,0,0);

	DEBUGP("workerthread %i is running (CDS prio:%lu, OS prio:%u)",i ,timer[i].pti->ulPriority, sched_param.sched_priority);

	while(timer[i].bRunWorker)
	{
		pthread_mutex_lock(&timer[i].workerMutex);
		pthread_cond_wait(&timer[i].workerCond,&timer[i].workerMutex);
		pthread_mutex_unlock(&timer[i].workerMutex);

		while(timer[i].bActive)
			sched_yield();
	}
	DEBUGP("workerthread %d is exiting",i);
	pthread_exit(0);
}
...
RTS_RESULT CDECL SysTimerClose(RTS_HANDLE hTimer)
{
	...
	timer[pTimerInfo->ulIRQ].bRunWorker = 0;
	pthread_cond_signal(&timer[pTimerInfo->ulIRQ].workerCond);
	pthread_join(timer[pTimerInfo->ulIRQ].worker, NULL);
...
RTS_RESULT CDECL SysTimerStart(RTS_HANDLE hTimer, unsigned long ulType)
{
	...
	pthread_mutex_init(&timer[pTimerInfo->ulIRQ].workerMutex,NULL);
	pthread_cond_init(&timer[pTimerInfo->ulIRQ].workerCond,NULL);
	...
	if (pthread_create(&timer[pTimerInfo->ulIRQ].worker, NULL, (void*(*)(void*))workerfunction,(void*)pTimerInfo->ulIRQ) != 0)
	{
		DEBUGP("pthread_create:%s",strerror(errno));
	}
	else
	{
		/* avoid waiting here until the worker is up and running. The problem is, that the timer might have already
		 * finished his work until the worker is started.
		 */
		res = ERR_OK;
		DEBUGP("worker %lu started",pTimerInfo->ulIRQ);
	}
