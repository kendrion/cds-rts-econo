...
/**
 * \file CmpPlcShellHandlerItf.h
 */
#include "CmpPlcShellHandlerItf.h"
...
#include "CmpEventMgrItf.h"
    ...
    {\
        ...
        { \
            ...
            if (pIBase != NULL) \
            { \
                pICmpPlcShell = (ICmpPlcShell *)pIBase->QueryInterface(pIBase, ITFID_ICmpPlcShell, &initResult); \
                pIBase->Release(pIBase); \
            } \
        } \
           \
    ...
    {\
        ...
        if (ERR_OK == importResult ) importResult = GET_PlcShellSetEof(0);\
          if (ERR_OK == importResult ) importResult = GET_PlcShellSkip(0);\
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpPlcShellHandler__Entry
#endif


#ifdef CPLUSPLUS

class CCmpPlcShellHandler : public ICmpPlcShellHandler 
{
    public:
        CCmpPlcShellHandler() : hCmpPlcShellHandler(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpPlcShellHandler()
        {
        }
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((ICmpPlcShellHandler *)this);            
            else if (iid == ITFID_ICmpPlcShellHandler)
                pItf = dynamic_cast<ICmpPlcShellHandler *>(this); 
            else
            {
                if (pResult != NULL)
    ...
    public:
        RTS_HANDLE hCmpPlcShellHandler;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpPlcShellHandler Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpPlcShellHandler_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpPlcShellHandler_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
