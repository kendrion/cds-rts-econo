/**
 * <name>IoDrvSubModulesSlot.c</name>
 * <description> 
 ...
 *	USAGE:
 * 
 *	Install the device description to the CODESYS device repository and use "Add Device" command to add the device "IoDrvSubModulesSlot" to your top level device (PLC).
...
#include "CmpStd.h"
#include "IoDrvSubModulesSlotDep.h"
...
static IoDrvSubModulesSlot s_IoDrvSubModulesSlot;
...
*/
{
	...
	if (pInitStruct->pfCMRegisterClass != NULL)
		pInitStruct->pfCMRegisterClass(COMPONENT_ID, CLASSID_CIoDrvSubModulesSlot);
...
{
	if (cid == CLASSID_CIoDrvSubModulesSlot)
	{
		IoDrvSubModulesSlot *pIoDrvSubModulesSlot = &s_IoDrvSubModulesSlot;
		IBase *pI = &pIoDrvSubModulesSlot->Base;
		pIoDrvSubModulesSlot->pIBase = &pIoDrvSubModulesSlot->Base;
		pI->bIEC = 0;
		pI->AddRef = AddRef;
		pI->Release = Release;
...
static void *CDECL QueryInterface(IBase *pBase, ITFID iid, RTS_RESULT *pResult)
{
	...
	{
		IoDrvSubModulesSlot *pC = (IoDrvSubModulesSlot *)pBase;
...
static RTS_RESULT CDECL HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)
{
	switch (ulHook)
	{
		...
		case CH_INIT2:
		{
			...
			/* first instance */
			pTmp = CAL_IoDrvCreate(hIoDrv, CLASSID_CIoDrvSubModulesSlot, 0, NULL);
			s_pIBase = (IBase *)pTmp;
...
static unsigned char s_ulyInputs[MAX_MODULES*MAX_CHANNELS];
static unsigned char s_ulyOutputs[MAX_MODULES*MAX_CHANNELS];
static int s_iInputChannel;
static int s_iOutputChannel;

STATICITF RTS_HANDLE CDECL IoDrvCreate(RTS_HANDLE hIIoDrv, CLASSID ClassId, int iId, RTS_RESULT *pResult)
{
	IoDrvInfo *pInfo;
	IoDrvSubModulesSlot *pIoDrvSubModulesSlot;
	...
	pIoDrvSubModulesSlot = (IoDrvSubModulesSlot *)hIIoDrv;
	pIoDrvSubModulesSlot->IoDrv.pBase = &pIoDrvSubModulesSlot->Base;
	pInfo = &pIoDrvSubModulesSlot->Info;
	...
	strcpy(pInfo->szDriverName, "IoDrvSubModulesSlot");
	strcpy(pInfo->szDeviceName, "ModuleIoDrvSubModulesSlot");
	...
	return (RTS_HANDLE)pIoDrvSubModulesSlot;
}
...
{
/* Find connector with moduleType="40109" as defined in device desciption file:
 ...
 * (child 257) Module IoDrvSubModulesSlot (parent 40109)
 *	                (child 40110) Module SubModuleAsLocalModule (parent 40111)
 *	                (child 40112) Module SlotSubModuleAsDeviceIdentification (parent 40113)
 *
*/
	int nCountBase = nCount;
	int nCountWork = 0;

	IoConfigConnector *pConnector = CAL_IoMgrConfigGetFirstConnector(pConnectorList, &nCount, 40109);
	IoDrvSubModulesSlot *pIoDrvSubModulesSlot = (IoDrvSubModulesSlot *)(hIoDrv); /* hIoDrv as returned from IoDrvCreate */
	IBase *pIBase = pIoDrvSubModulesSlot->pIBase;
	/*ICmpIoDrv *pIIoDrv = pIoDrvSubModulesSlot->pIoDrv;*/
	...
	if (pConnector != NULL)
	{
		...
		if (pConnector->hIoDrv == 0)
		{
			...
			while (pChild != NULL)
			{
				...
				/* This is the local slot submodule */
				if (pChildChild->wType == 40111)
				{
					/* This module has input channels */
					for(i=0; i < MAX_CHANNELS; i++)
					{
						pParameter = CAL_IoMgrConfigGetParameter(pChildChild, 1000 + i);
						if (pParameter != NULL)
						{
							pParameter->dwDriverSpecific = (RTS_IEC_BYTE *)&s_ulyInputs[s_iInputChannel];
							s_iInputChannel++;
						}
						else
							break;
				...
				/* This is the slot submodule defined in separate file */
				if (pChildChild->wType == 40113)
				{
					/* This module has output channels */
					for(i=0; i < MAX_CHANNELS; i++)
					{
						pParameter = CAL_IoMgrConfigGetParameter(pChildChild, 1000 + i);
						if (pParameter != NULL)
						{
							pParameter->dwDriverSpecific = (RTS_IEC_BYTE *)&s_ulyOutputs[s_iOutputChannel];
							s_iOutputChannel++;
						}
						else
							break;
