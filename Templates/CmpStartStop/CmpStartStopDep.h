 ...
 *  </copyright>
 */
#ifndef _CMPSTARTSTOPDEP_H_
#define _CMPSTARTSTOPDEP_H_

#define COMPONENT_NAME "CmpStartStop" COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpStartStop)
#define COMPONENT_NAME_UNQUOTED CmpStartStop
...
#include "SysMsgQItf.h"
    ...
    {\
        ...
        if (ERR_OK == importResult ) TempResult = GET_SysMemIsValidPointer(CM_IMPORT_OPTIONAL_FUNCTION);\
          ...
          if (ERR_OK == importResult ) importResult = GET_AppStartApplication(0);\
          if (ERR_OK == importResult ) importResult = GET_AppStopApplication(0);\
          if (ERR_OK == importResult ) importResult = GET_AppGetNumOfApplications(0);\
...
#ifndef CMPSTARTSTOP_DISABLE_EXTREF
#define EXPORT_EXTREF_STMT \
          
#else
#define EXPORT_EXTREF_STMT
#endif
#ifndef CMPSTARTSTOP_DISABLE_EXTREF2
#define EXPORT_EXTREF2_STMT \
          
#else
#define EXPORT_EXTREF2_STMT
#endif
#if !defined(CMPSTARTSTOP_DISABLE_CMPITF) && !defined(STATIC_LINK) && !defined(CPLUSPLUS) && !defined(CPLUSPLUS_ONLY)
...
#define USE_STMT \
    ...
    USE_SysMsgQCreate      \
    USE_SysMsgQSend      \
    USE_AppGetNumOfApplications      \
    USE_AppStopApplication      \
    USE_AppStartApplication      \
...
#define USEEXTERN_STMT \
    ...
    EXT_SysMsgQCreate  \
    EXT_SysMsgQSend  \
    EXT_AppGetNumOfApplications  \
    EXT_AppStopApplication  \
    EXT_AppStartApplication  \
    ...
    #define ComponentEntry CmpStartStop__Entry
#endif


#ifdef CPLUSPLUS

class CCmpStartStop : public ICmpEventCallback 
{
    public:
        CCmpStartStop() : hCmpStartStop(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpStartStop()
        {
        }
    ...
    public:
        RTS_HANDLE hCmpStartStop;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpStartStop Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpStartStop_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpStartStop_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
