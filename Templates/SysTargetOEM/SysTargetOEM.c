/**
 * <name>SysTargetOEM.c</name>
 * <description> 
 *	Example to handle the target identification of runtime system.
 *	NOTE:
 *	All content is typically specified in targetdefines.h an are used here as macros.
 *
 *	USAGE:
...
/* The following include is required to overwrite the IDs from sysdefines.h by the OEM-specific ones. For automatic build out of a starterpackage.*/
/* In case of an error message like "cannot open include file", the build is not made from a startepackage directory structure. Create an empty file to compile with defaults.*/
...
#ifdef CPLUSPLUS
	static CSysTarget *s_pCSysTarget;
#endif
	...
	switch (ulHook)
	{
		...
		case CH_COMM_CYCLE:
			...
			if (bDebug)
			{
				char sz[255];
				char *psz = sz;
				int nMaxLen = sizeof(sz);

				SysTargetGetSerialNumber(&psz, &nMaxLen);
			}
			*/
			}
			break;
...
/**
 * <description>
 *  Get a human readable name for the target which is displyed for example in the online scan.
 *	The name can be configured by the following setting: SYSTARGETKEY_STRING_GET_NODENAME = "NodeName"
 *	See SysTargetItf.h for details.
 * </description>
...
RTS_RESULT CDECL SysTargetGetConfiguredNodeName(RTS_WCHAR *pwszName, RTS_SIZE *pnMaxLength)
{
	if (pnMaxLength != NULL)
	{
		int iLen = (int)*pnMaxLength;
		RTS_RESULT Result = CAL_SettgGetWStringValue((const char*)COMPONENT_NAME, (const char*)SYSTARGETKEY_WSTRING_GET_NODENAME_UNICODE, (RTS_WCHAR *)pwszName, (int *)&iLen, (RTS_WCHAR *)RTS_WTEXT_EMPTY, 0);
		if (Result == ERR_OK)
		{
			*pnMaxLength = iLen + 1;
		}
...
RTS_RESULT CDECL SysTargetGetDeviceName(RTS_WCHAR *pwszName, RTS_SIZE *pnMaxLength)
{
	...
	Result = CAL_SettgGetWStringValue((const char*)COMPONENT_NAME, (const char*)szKey, (RTS_WCHAR *)pwszName, (int *)&iLen, (RTS_WCHAR *)RTS_WTEXT_EMPTY, 0);
	if (Result == ERR_OK)
	{
		*pnMaxLength = iLen + 1;
	}
...
RTS_RESULT CDECL SysTargetGetSerialNumber_Secure(char **ppszSerialNumber, RTS_I32 *pnMaxLen)
{
	...
	*pnMaxLen = 0;
	...
	/* TODO: For licensing, return here a _secure_ serial number of the target that is robust against cracking attacks!!! */

	/*
		EXAMPLE:
		static char s_szTargetSerialNumber[255] = {0};

		TODO: Retrieve the secure serial number

		*pnMaxLen = MIN((int)strlen(s_szTargetSerialNumber) + 1, *pnMaxLen);
...
RTS_RESULT CDECL SysTargetCheckIdent(SysTargetIdent *pTargetIdentReq, SysTargetIdent *pTargetIdent)
{
	...
	{
		SysTargetGetType(&ti.ulTargetType);
		SysTargetGetId(&ti.ulTargetId);
		SysTargetGetVersion(&ti.ulTargetVersion);
		pTargetIdent = &ti;
	}
...
void CDECL CDECL_EXT systargetgetversion(systargetgetversion_struct *p)
{
	...
	if (CHK_SIL2CheckCallerContext && CAL_SIL2CheckCallerContext(RTS_SIL2_CALLERCTX_UNSAFE) != ERR_OK)
		return;
#endif
	p->SysTargetGetVersion = SysTargetGetVersion(p->pulVersion);
}
...
void CDECL CDECL_EXT systargetgetid(systargetgetid_struct *p)
{
	...
	if (CHK_SIL2CheckCallerContext && CAL_SIL2CheckCallerContext(RTS_SIL2_CALLERCTX_UNSAFE) != ERR_OK)
		return;
#endif
	p->SysTargetGetId = SysTargetGetId(p->pulTargetId);
}
...
void CDECL CDECL_EXT systargetgettype(systargetgettype_struct *p)
{
	...
	if (CHK_SIL2CheckCallerContext && CAL_SIL2CheckCallerContext(RTS_SIL2_CALLERCTX_UNSAFE) != ERR_OK)
		return;
#endif
	p->SysTargetGetType = SysTargetGetType(p->pulType);
}
...
void CDECL CDECL_EXT systargetgettype2(systargetgettype2_struct *p)
{
	...
	if (CHK_SIL2CheckCallerContext && CAL_SIL2CheckCallerContext(RTS_SIL2_CALLERCTX_UNSAFE) != ERR_OK)
		return;
#endif
	p->SysTargetGetType2 = SysTargetGetType(p->pulType);
}
...
void CDECL CDECL_EXT systargetgetversion2(systargetgetversion2_struct *p)
{
	...
	if (CHK_SIL2CheckCallerContext && CAL_SIL2CheckCallerContext(RTS_SIL2_CALLERCTX_UNSAFE) != ERR_OK)
		return;
#endif
	p->SysTargetGetVersion2 = SysTargetGetVersion(p->pulVersion);
}
...
void CDECL CDECL_EXT systargetgetid2(systargetgetid2_struct *p)
{
	...
	if (CHK_SIL2CheckCallerContext && CAL_SIL2CheckCallerContext(RTS_SIL2_CALLERCTX_UNSAFE) != ERR_OK)
		return;
#endif
	p->SysTargetGetId2 = SysTargetGetId(p->pulTargetId);
}
...
void CDECL CDECL_EXT systargetgetserialnumber(systargetgetserialnumber_struct *p)
{
	...
	if (CHK_SIL2CheckCallerContext && CAL_SIL2CheckCallerContext(RTS_SIL2_CALLERCTX_UNSAFE) != ERR_OK)
		return;
#endif
	p->SysTargetGetSerialNumber = SysTargetGetSerialNumber(p->ppsSerialNumber, (int *)p->pnMaxLen);
}
#endif
