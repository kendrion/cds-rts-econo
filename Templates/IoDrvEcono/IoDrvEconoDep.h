...
#define COMPONENT_NAME "IoDrvEcono" COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_IoDrvEcono)
#define COMPONENT_NAME_UNQUOTED IoDrvEcono
...
#define CMPID_IoDrvEcono		0x2000								/* NOTE: START HERE WITH YOUR COMPONENTIDS (see CmpItf.h */
#define CLASSID_CIoDrvEcono	ADDVENDORID(CMP_VENDORID, 0x2000)	/* NOTE: START HERE WITH YOUR CLASSIDS (see CmpItf.h */
#define ITFID_IIoDrvEcono		ADDVENDORID(CMP_VENDORID, 0x2000)	/* NOTE: START HERE WITH YOUR INTERFACEIDS (see CmpItf.h */
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry IoDrvEcono__Entry
#endif


#ifdef CPLUSPLUS

class CIoDrvEcono : public IIoDrvEcono 
{
    public:
        CIoDrvEcono() : hIoDrvEcono(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CIoDrvEcono()
        {
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((IIoDrvEcono *)this);            
            else if (iid == ITFID_IIoDrvEcono)
                pItf = dynamic_cast<IIoDrvEcono *>(this); 
            else
            {
                if (pResult != NULL)
    ...
    public:
        RTS_HANDLE hIoDrvEcono;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* IoDrvEcono Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT IoDrvEcono_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF IoDrvEcono_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
