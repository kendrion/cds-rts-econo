...
#define COMPONENT_NAME "CmpEconoTemplate" COMPONENT_NAME_POSTFIX
#define COMPONENT_ID    ADDVENDORID(CMP_VENDORID, CMPID_CmpEconoTemplate)
#define COMPONENT_NAME_UNQUOTED CmpEconoTemplate
...
#define CMPID_CmpEconoTemplate		0x2000								/* NOTE: START HERE WITH YOUR COMPONENTIDS (see CmpItf.h */
#define CLASSID_CCmpEconoTemplate	ADDVENDORID(CMP_VENDORID, 0x2000)	/* NOTE: START HERE WITH YOUR CLASSIDS (see CmpItf.h */
#define ITFID_ICmpEconoTemplate		ADDVENDORID(CMP_VENDORID, 0x2000)	/* NOTE: START HERE WITH YOUR INTERFACEIDS (see CmpItf.h */
...
#define USEEXTERN_STMT \
    ...
    #define ComponentEntry CmpEconoTemplate__Entry
#endif


#ifdef CPLUSPLUS

class CCmpEconoTemplate : public ICmpEconoTemplate 
{
    public:
        CCmpEconoTemplate() : hCmpEconoTemplate(RTS_INVALID_HANDLE), iRefCount(0)
        {
        }
        virtual ~CCmpEconoTemplate()
        {
        }
        ...
        virtual void* QueryInterface(IBase *pIBase, ITFID iid, RTS_RESULT *pResult)
        {
            void *pItf;
            if (iid == ITFID_IBase)
                pItf = dynamic_cast<IBase *>((ICmpEconoTemplate *)this);            
            else if (iid == ITFID_ICmpEconoTemplate)
                pItf = dynamic_cast<ICmpEconoTemplate *>(this); 
            else
            {
                if (pResult != NULL)
    ...
    public:
        RTS_HANDLE hCmpEconoTemplate;
        int iRefCount;
};

#endif /*CPLUSPLUS*/
#ifdef RTS_COMPACT_MICRO
/* CmpEconoTemplate Declarations for uRTS */
...
#define USE_HOOK_FUNCTION	RTS_RESULT CmpEconoTemplate_HookFunction(RTS_UI32 ulHook, RTS_UINTPTR ulParam1, RTS_UINTPTR ulParam2)\
{\
...
#define USE_EXPORT_EXTREF const CMP_EXT_FUNCTION_REF CmpEconoTemplate_ExternalsTable[] = \
	{\
		EXPORT_EXTREF_STMT\
		EXPORT_EXTREF2_STMT\
